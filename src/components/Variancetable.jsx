  ...
  async function fetchData() {
    ...
    try {
      ...
      const formatted = result.map((row) => ({
        ...
        serviceNo: row.SCNO,
        average: row.average.consumption,
        peak: {
          hour: row.high.hour,
          value: row.high.avg_consumption,
          percent: `${row.high.percent_increase_from_avg.toFixed(2)}%`,
        },
        low: {
          hour: row.low.hour,
          value: row.low.avg_consumption,
          percent: `-${row.low.percent_decrease_from_avg.toFixed(2)}%`,
        },
      }))

      setData(formatted)
      localStorage.setItem(CACHE_KEY, JSON.stringify({ timestamp: new Date().getTime(), data: formatted }))
    } catch (err) {
          ...
          <table className="w-full border border-gray-300 text-sm text-center">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th rowSpan={2} className="border px-3 py-2 align-middle">Average (Wh)</th>
                <th colSpan={3} className="border px-3 py-2">Peak</th>
                <th colSpan={3} className="border px-3 py-2">Low</th>
              </tr>
              <tr className="bg-gray-100">
                {["Peak", "Low"].map((type) => (
                  <React.Fragment key={type}>
                    <th className="border px-3 py-2">Hour</th>
                    <th className="border px-3 py-2">Value(Wh)</th>
                    <th className="border px-3 py-2">
                      % {type === "Peak" ? <span className="text-green-600">▲</span> : <span className="text-red-600">▼</span>}
                    </th>
                  </React.Fragment>
                ))}
              </tr>
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="border px-3 py-2 font-medium">{row.peak.percent}</td>

                  {/* Low */}
                  <td className="border px-3 py-2">{row.low.hour}</td>
                  <td className="border px-3 py-2">{row.low.value}</td>
                  <td className="border px-3 py-2 font-medium">{row.low.percent}</td>
                </tr>
              ))}
            </tbody>
          </table>
